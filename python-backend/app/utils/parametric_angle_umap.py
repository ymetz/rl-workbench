...
class ParametricAngleUMAP(UMAP):
    def __init__(
            self,
            optimizer=None,
            batch_size=None,
    ...
    def fit_transform(self, X, y=None, precomputed_distances=None, action_angles=None):
        ...
        if action_angles is not None:
            self.action_angles = tf.convert_to_tensor(action_angles, dtype_hint=tf.float32)
    ...
    def _define_model(self):
        ...
        outputs["angle"] = tf.keras.layers.Lambda(
            lambda x: x, name="angle"
        )(embedding_to)
...
def angle_loss(ref_action_angles):
    ...
    def loss(y, z_x):
        ...
        # First half are the previous, second half are the next steps
        y = tf.reshape(y, [-1])
        y = tf.clip_by_value(y, 0, ref_action_angles.shape[0] - 1)[:-1]

        z_x1, z_x2 = tf.split(z_x, num_or_size_splits=2, axis=0)
        angles = tf.math.atan2(z_x1[1:, 1] - z_x2[:-1, 1], z_x1[1:, 0] - z_x2[:-1, 0])
        angles = (angles + 2 * tf.constant(np.pi)) % (2 * tf.constant(np.pi))
        # angles = tf.gather(angles, indices=y)

        return tf.math.reduce_euclidean_norm(tf.gather(ref_action_angles, indices=y) - angles, axis=-1)
...
def construct_edge_dataset(
    ...
    def gather_X(edge_to, edge_from):
        ...
        if gather_indices_in_python:
            ...
            shifted_edge_to_batch = tf.py_function(gather_index, [tf.clip_by_value(edge_to - 1, 0, tf.int32.max)], [tf.float32])[0]
            edge_from_batch = tf.py_function(gather_index, [edge_from], [tf.float32])[0]
            shifted_edge_from_batch = \
            tf.py_function(gather_index, [tf.clip_by_value(edge_from - 1, 0, tf.int32.max)], [tf.int32])[0]
        else:
            edge_to_batch = tf.gather(X, edge_to)
            shifted_edge_to_batch = tf.gather(X, tf.clip_by_value(edge_to - 1, 0, tf.int32.max))
            edge_from_batch = tf.gather(X, edge_from)
            shifted_edge_from_batch = tf.gather(X, tf.clip_by_value(edge_from - 1, 0, tf.int32.max))
        return tf.concat([edge_to_batch, shifted_edge_to_batch], axis=0), \
