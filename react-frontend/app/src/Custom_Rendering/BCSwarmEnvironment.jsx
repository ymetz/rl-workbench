...
export default class BCSwarmEnvironment extends Component {
    ...
    updateData(newData, newSecondaryData, means) {
        ...
        newData.concat(newSecondaryData).forEach((p, i) => {
            // p[0]: x, p[1]: y, p[2]: angle, p[3]: speed
            const tail_direction = p[2] - Math.PI - 2 * Math.PI * Math.floor(p[2] / (2 * Math.PI));
            const tail_length = p[3] * 2; // 50 means length is propotial to covered distance per sec.
            const tail_endpoint = [Math.cos(tail_direction) * tail_length, Math.sin(tail_direction) * tail_length];
            ...
            data_point
                ...
                .attr(
                    'd',
                    'M' + xScale(0) + ' ' + yScale(0) + ' L' + xScale(tail_endpoint[0]) + ' ' + yScale(tail_endpoint[1])
                )
            ...
            if (i == 0) {
                const nr_of_neighbors = Math.trunc((p.length - 4) / 3);
                const end_point_list = [];
                for (let i = 0; i < nr_of_neighbors; i++) {
                    const n_tail_direction = p[4 + i * 3 + 1];
                    const n_tail_length = p[4 + i * 3];
                    const n_tail_endpoint = [
                        Math.cos(n_tail_direction) * n_tail_length,
                        Math.sin(n_tail_direction) * n_tail_length,
                    ];
                ...
                data_point
                    ...
                    .append('path')
                    .attr('stroke', 'orange')
                    .attr('stroke-width', 0.5 / current_zoom_factor)
                    .attr(
                        'd',
                        (d) =>
                            'M' +
    ...
    drawChart(dims, means) {
        ...
        //view.append("rect").attr("x", 0).attr("y", 0).attr("height", svgHeight).attr("width", svgWidth).style("opacity", "0");
        ...
        function saveZoom() {
            const zoom_factor = event !== null ? event.transform.k : 1.0;
            _self.setState({
                dims: {
